...
DROP FUNCTION IF EXISTS public.create_pix_payment(numeric, text, text);
DROP FUNCTION IF EXISTS public.create_pix_payment(numeric, text, text, text);
DROP FUNCTION IF EXISTS public.create_pix_payment_v1(numeric, text, text, text);
...
-- Create final version of PIX payment function with unique name
CREATE OR REPLACE FUNCTION public.generate_pix_payment_v5(
  p_amount numeric,
...
GRANT EXECUTE ON FUNCTION public.generate_pix_payment_v5 TO anon;
GRANT EXECUTE ON FUNCTION public.generate_pix_payment_v5 TO authenticated;