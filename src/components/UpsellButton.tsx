...
export default function UpsellButton({ 
  ...
  const handleClick = async () => {
    try {
      ...
      // Create new payment intent for upsell
      const { data: intentData, error: intentError } = await supabase
        .rpc('create_upsell_payment', {
          p_offer_id: offerId,
  ...
  return (
    <div className="space-y-4">
      <button
        onClick={handleClick}
        disabled={loading}
        className={`flex items-center justify-center gap-2 w-full px-6 py-3 text-lg font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 disabled:opacity-50 ${className}`}
      >
        {loading ? (
          'Processing...'
        ) : (
